...
Begin VB.Form f3 
   ...
   Begin VB.Label legT 
      Alignment       =   2  'Center
      BackColor       =   &H00C0FFC0&
      Caption         =   "Cadastro de nova biografia"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
         Charset         =   0
...
If MsgBox("Deseja mesmo escluir esta biografia?", vbYesNo, "Hemeroteca") = vbYes Then
    If øCon.State = 1 Then
        øCon.Execute "Delete * From tabBiografias Where Cod = " & CDbl(Me.Tag)
    End If
...
If øCon.State = 1 Then
    If botExcluir.Visible = True Then
        øCon.Execute "Update tabBiografias Set " _
    ...
    Else
        øCon.Execute "Insert Into tabBiografias " _
            & "(Assunto, AssuntoSA, Pasta, Comentário) Values (" _
            & Ap(Assunto) & ", " & Ap(AssuntoSA) & ", " & Ap(Pasta) & ", " _
            & Ap(Comentário) & ")"
    End If
End If
