...
Begin VB.Form f2 
   ...
   Begin VB.Label legT 
      Alignment       =   2  'Center
      BackColor       =   &H008080FF&
      Caption         =   "Cadastro de novo assunto do fichário"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
         Charset         =   0
...
If MsgBox("Deseja mesmo excluir este assunto?", vbYesNo, "Hemeroteca") = vbYes Then
    If øCon.State = 1 Then
        øCon.Execute "Delete * From tabFichário Where Cod = " & CDbl(Me.Tag)
    End If
...
Dim Assunto As String, AssuntoSA As String, Pasta As String, Subpasta As String
Assunto = Trim(txtAssunto)
AssuntoSA = RetirarAcento(Assunto)
Pasta = Trim(txtPasta)
Subpasta = Trim(txtSubpasta)
...
If øCon.State = 1 Then
    If botExcluir.Visible = True Then
        øCon.Execute "Update tabFichário Set " _
        & "Assunto = " & Ap(Assunto) _
        & ", AssuntoSA = " & Ap(AssuntoSA) _
        & ", Pasta = " & Ap(Pasta) _
        & ", Subpasta = " & Ap(Subpasta) _
        & " Where Cod = " & CDbl(Me.Tag)
    Else
        øCon.Execute "Insert Into tabFichário " _
        & "(Assunto, AssuntoSA, Pasta, Subpasta) Values (" _
        & Ap(Assunto) & ", " & Ap(AssuntoSA) & ", " _
        & Ap(Pasta) & ", " & Ap(Subpasta) & ")"
    End If
End If
